...
public class CategoriesController {
    ...
    public ResponseEntity<CategoriesDTO> addCategory(@RequestPart("categoriesDTO") String categoriesJson,
                                                     @RequestPart(value = "image", required = false) MultipartFile image) throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        CategoriesDTO request = mapper.readValue(categoriesJson, CategoriesDTO.class);

        return ResponseEntity.ok(categoriesService.addCate(request, image));
