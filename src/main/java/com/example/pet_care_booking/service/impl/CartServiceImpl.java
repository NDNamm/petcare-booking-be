    ...
    private CartDTO getCartDTO(Cart cart, CartDTO cartDTO) {
        ...
        for (CartItem items : cart.getItems()) {
            ...
            ProductDTO productDTO = ProductDTO.builder()
                    ...
                    .price(variantRepository.findPriceBySizeAndProduct(items.getVariant().getSize(), product))
    ...
    private void addProductToCart(CartItemDTO cartItemDTO, Cart cart) {
        ...
        Optional<CartItem> existingItem = cart.getItems().stream()
                .filter(item -> item.getProduct().getId().equals(product.getId())
                        && item.getVariant().getSize().equals(cartItemDTO.getSize()))
                .findFirst();
        Variants variant = variantRepository.findBySizeAndProduct(cartItemDTO.getSize(), product)
                .orElseThrow(() -> new AppException(ErrorCode.PRODUCT_NOT_FOUND));
        if (variant.getStock() < cartItemDTO.getQuantity() ||
        ...
        if (existingItem.isPresent()) {
            ...
            cartItem = CartItem.builder()
                    ...
                    .price(price)
                    .build();
            cartItem.updateQuantityAndPrice(addedQuantity, price);
        }
